...
import logging
from backend.risk_engine import RiskEngine  # IMPORT NEW ENGINE
...
class MLService:
    def __init__(self, model_path="model/model.pkl", tfidf_path="model/tfidf.pkl"):
        ...
        self.engine = RiskEngine()  # INITIALIZE ENGINE
    ...
    def predict(self, job_data, processed_df):
        """
        Returns: (Verdict, Probability, Human_Report)
        """
        if not self.model or not self.tfidf:
        ...
        # 4. Generate Human Explanation (Kiara's Logic)
        report = self.engine.get_comprehensive_report(processed_df.iloc[0])
        
        return verdict, float(prob), report
